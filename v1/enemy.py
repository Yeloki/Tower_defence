...
class Enemy:
    ...
    def __init__(self, game_map: list, wave=1, difficult=1) -> None:
        ...
        self.max_hp = self.hp
        self.x, self.y = game_map[self.i].point1()
    ...
    def move(self) -> None:
        ...
        if self.freeze_flag:
            ...
            speed = self.speed
        if distance((self.x, self.y), self.game_map[self.i].point2()) <= speed * 2:
            if self.i + 1 == len(self.game_map):
                self.current_status = 'ENEMY_STATUS_TO_GET_TO_BASE'
            else:
                if distance((self.x, self.y), self.game_map[self.i].point2()) > speed:
        ...
        try:
            k = 0
            if self.game_map[self.i].point1()[0] - self.game_map[self.i].point2()[0] == 0 and \
                    self.game_map[self.i].point1()[1] - self.game_map[self.i].point2()[1] < 0:
                self.angle = (180 + randint(-2, 2)) % 360
            else:
                tan = (self.game_map[self.i].point1()[1] - self.game_map[self.i].point2()[1]) / \
                      (self.game_map[self.i].point1()[0] - self.game_map[self.i].point2()[0])
                if ((self.game_map[self.i].point1()[1] - self.game_map[self.i].point2()[1]) > 0 and
                    (self.game_map[self.i].point1()[0] - self.game_map[self.i].point2()[0]) > 0) or \
                        ((self.game_map[self.i].point1()[1] - self.game_map[self.i].point2()[1]) <= 0 < (
                                self.game_map[self.i].point1()[0] - self.game_map[self.i].point2()[0])):
                    k = -180
